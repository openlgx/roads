...
The app is now a **local-first Android collector plus experimental on-device analysis and review** (Room **v5**, heuristic roughness windows, exports with derived artifacts, optional **hosted alpha** upload queue and road-pack-based export filtering). It is **not** yet a full production networked product: **labelling/training pipelines, calibration to IRI, hardened production ops, and council rollout** remain future work—but **ingestion scaffolding** (Neon + Supabase Edge Functions + WorkManager upload) now exists in-tree.

**Authoritative phased plan:** see [ROADMAP.md](ROADMAP.md). **Server shape (Neon metadata, Storage blobs, Edge Functions):** see [docs/backend.md](docs/backend.md). **Normative upload API:** [docs/api-contract.md](docs/api-contract.md). **Operator setup:** [docs/setup-hosted-alpha.md](docs/setup-hosted-alpha.md).
