...
**Debug pilot bootstrap (alpha ergonomics, no secrets in git):** Debug APKs set `BuildConfig.PILOT_BOOTSTRAP_ENABLED` and, on **first launch only**, seed DataStore with the Yass Valley / alpha-pilot council URL, slugs, and Neon-style project/device UUIDs (`PilotBootstrapConfig`). This is a **one-shot**: `pilot_bootstrap_applied` prevents overwriting later edits in Settings. **DEVICE_UPLOAD API key** is never committed: inject it for local debug builds using either (1) `roads.pilot.upload.key=` in **`local.properties`** (already git-ignored), or (2) environment variable **`ROADS_PILOT_UPLOAD_KEY`** at Gradle time. If the key is absent, bootstrap still fills URLs/UUIDs but leaves **hosted upload disabled** (`uploadEnabled = false`) so the app stays local-first and opens without a key. Release builds set `PILOT_BOOTSTRAP_ENABLED = false` and empty injected key. See Diagnostics and Settings → hosted diagnostics for bootstrap status, expected road-pack path, and upload readiness.
