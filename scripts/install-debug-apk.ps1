<#
.SYNOPSIS
  Install the debug APK on a USB-connected (or adb-visible) device.

.PARAMETER DeviceSerial
  Optional. When multiple devices are attached, set this to the serial from `adb devices`.

.PARAMETER ApkPath
  Optional. Defaults to app\build\outputs\apk\debug\app-debug.apk
...
if (-not (Test-Path -LiteralPath $apk)) {
    ...
    Write-InstallLog "INFO" "Build first: .\scripts\build-debug-apk.ps1"
    exit 1
}
...
Write-InstallLog "INFO" "adb install -r"
...
Write-InstallLog "OK" "Installed org.openlgx.roads (debug)"
